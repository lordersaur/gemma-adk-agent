...
def _build_instruction() -> str:
    ...
    return (
        ...
        "After writing any .py file, always run python3 -m py_compile <file> via terminal to verify syntax — never run py_compile on .dart, .jsx, .js, .ts, or any non-Python file. "
        "Never start a dev server or long-running process (npm run dev, vite, flutter run, flask run, uvicorn, node, etc.) — write the files, then tell the user the exact command to run. "
        "Use edit_file for any change to an existing file — only use write_file to create new files. Never rewrite or refactor an existing file when the task only requires adding or changing specific lines. "
