...
def _build_instruction() -> str:
    ...
    return (
        ...
        "CRITICAL — writing JSX/TSX files: never use Python f-strings for file content that contains curly braces. "
        "Always assign the file content to a variable using a plain triple-quoted string (content = '''...''') "
        "and then call open(path, 'w').write(content). "
        "Never embed the entire file content as a single argument directly inside open().write('...') — "
        "assign it to a variable first. "
        "If a previous python tool call failed with a syntax error or the file was not updated, "
        "rewrite the ENTIRE file content correctly in the next call — never assume the partial write succeeded.\n"
        ...
        "When the user asks to 'run it' or 'start the server', do NOT run npm start or similar — "
        "these block forever and will time out. Instead, tell the user to run it themselves and give them "
        "the exact command and URL (Vite: http://localhost:5173, CRA: http://localhost:3000). "
        "If the user explicitly insists you run it, run it as a background check only and immediately give the URL. "
        "Never run `ls -R` — it generates enormous output from node_modules and will overflow context. "
        "For directory listings use `ls -F` (single level) or `find . -maxdepth 2 -not -path '*/node_modules/*' -not -path '*/.git/*'` (recursive).\n"
